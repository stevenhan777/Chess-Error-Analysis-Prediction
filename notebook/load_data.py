...
def _fetch_games(username, perf_types, max_games):
    ...
    if resp.status_code != 200:
        raise ConnectionError(f"Lichess API returned {resp.status_code}: {resp.text[:200]}")
...
def _classify_piece_move(move):
    clean = move.strip('+#')
    if clean.startswith('O-O'):
        return {'is_pawn_move':0,'is_knight_move':0,'is_bishop_move':0,
...
def _track_board_state(moves_san):
    board_states = []
    pieces_remaining = 32
...
def _to_move_level(games):
    ...
    for game in games:
        ...
        board_states = _track_board_state(moves)
        evals_list   = []
        ...
        for i, move in enumerate(moves):
            ...
            if time_left is not None:
                ...
                if color == 'white':
                    if i >= 2 and i - 2 < len(clocks):
                        prev_time = clocks[i - 2]
                        # FIXED: Don't add increment - it's already in time_left
                        time_spent = prev_time - time_left
                    elif i == 0:
                        # First move: initial time - time left
            ...
            records.append({
                ...
                'eval_volatility': vol,
                **{k: bs.get(k) for k in [
                    'pieces_remaining','material_density','num_minor_pieces',
                    'num_rooks','num_queens','num_pawns','complexity_material_score',
                    'white_castled_king','white_castled_queen','black_castled_king',
                    'black_castled_queen','queen_present',
...
def load_and_process(username, perf_type, max_games):
    perf_types = ['blitz'] if perf_type == 'blitz' else ['rapid', 'classical']

    print(f"Fetching up to {max_games} games for {username} ({', '.join(perf_types)})…")
    raw_games = _fetch_games(username, perf_types, max_games)

    filtered = [
        g for g in raw_games
        if 'analysis' in g
        and len(g.get('analysis', [])) > 0
    ]
    print(f"Total fetched: {len(raw_games)} | With analysis: {len(filtered)}")
...
# run for username 'clownmitmuetze' for rapid_classical and 'DoktorAvalanche' for blitz
if __name__ == "__main__":
    filtered = load_and_process('DoktorAvalanche',  'blitz', 100000)
    df = _to_move_level(filtered)
    base = os.path.dirname(os.path.abspath(__file__))
    df.to_csv(os.path.join(base,'data/DoktorAvalanche_blitz_loaded.csv'), index = False)
